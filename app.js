...
// Complete the exercise in the space below:
inventorNames = inventors.map((obj) => ({
  first: obj.first,
  last: obj.last,
}));
...
// sortedByBirthYear = inventors.sort(ascendingOrder);
...
// Complete the exercise in the space below:
inventorNamedAda = inventors.find((obj) => obj.first === "Ada");
...
// Complete the exercise in the space below:
isAdultPresent = devs.some((obj) => {
  const todayYear = new Date().getFullYear();
  const devAge = todayYear - obj.year;
  return devAge >= 18;
});
...
isEveryone19OrOlder = devs.every((obj) => {
  const todayYear = new Date().getFullYear();
  const devAge = todayYear - obj.year;
  return devAge >= 19;
});
...
// Complete the exercise in the space below:
commentById = comments.find((obj) => obj.id === 823423);
...
// Complete the exercise in the space below:
idx = comments.findIndex((obj) => obj.id === 123523);
...
// Complete the exercise in the space below:
totalYearsLived = inventors.reduce((sumTotal, obj) => {
  const yearsLived = obj.passed - obj.year;
  return (sumTotal += yearsLived);
}, totalYearsLived);
...
travelMethodCounts = travelMethods.reduce((accumulator, travelMethod) => {
  // accumulator is an empty object initialised for this reduce method
  if (accumulator[travelMethod]) {
    // if accumulator does not have a key with the variable travelMethod (e.g. car, truck, bike), accumulator[travelMethod] returns undefined -> falsey value
    accumulator[travelMethod]++; // add count for the value of accumulator[travelMethod]
  } else {
    // initialise that KVP within the travelMethodCounts object
